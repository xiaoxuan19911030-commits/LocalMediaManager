...
- 大规模本地媒体库的浏览、搜索与详情展示
- 元数据显示与同步（标题、演员、导演、厂商、系列、标签、NFO、Provider）
- 用户个人数据管理（评分、收藏、自定义标签、播放历史、演员显示排序、图片选择）
...
2. **用户个人数据优先** — 评分、收藏、自定义标签、播放历史、演员显示排序、备注与图片选择**高于**刮削、插件和 AI 结果；影片元数据本身由刮削、NFO 导入和同步维护。
...
- 基础浏览、搜索、个人数据管理和标签管理**不依赖**任何外部服务。
...
- 评分、收藏、自定义标签、播放历史、演员显示排序、备注、图片选择**永远高于**自动刮削结果。
- 元数据同步、NFO 导入和图片自动同步默认不覆盖用户个人数据、锁定图片或用户 NFO。
- 收藏、评分、自定义标签、演员显示排序和播放历史是真实用户数据，必须持久化并在重启后保持。
...
**ProductWriter** — 用户状态与安全结构变更：
- 评分、收藏、自定义标签 CRUD、演员显示排序/修复
- 批量操作、删除预览/执行
...
| 规则 | Next 实现要点 |
|------|--------------|
| **用户个人数据优先** | 同步/刮削不覆盖评分、收藏、自定义标签、播放历史、演员显示排序、锁定图 |
...
### 13.0 Metadata Ownership（元数据归属）

**长期产品决策（DEC-013）：** LMM 不承担影片元数据手动编辑器职责。影片元数据的权威来源是刮削、NFO 导入和元数据同步；LMM 负责展示这些元数据，并提供重新同步、重新刮削、NFO 导入/导出和图片工作流。

**不提供手动编辑入口：** 标题、原始标题、番号、演员添加/删除/搜索、导演、厂商、系列、影片标签/Genre、简介、上映日期、年份、时长、显示标题、自定义标题或第二标题。MovieWall 和详情页直接显示刮削/NFO/同步得到的标题与原标题；元数据错误时通过重新刮削或重新导入 NFO 修正来源数据。

**LMM 维护的个人数据：** 星级评分、收藏、自定义标签、海报调整、人工裁切、演员显示排序、播放次数、最后播放时间、最近播放，以及未来明确提出后才开发的用户备注和 AI 推荐行为数据。这些数据与刮削元数据分离，重新同步元数据时不得覆盖。Image SetAs（设为海报 / 缩略图 / 横幅）已按 DEC-014 产品取消；图片资源由 MetaTube 刮削、NFO 导入和元数据同步统一维护，用户侧保留查看、放大、人工裁切、刷新和重新下载图片。

**演员边界：** 演员实体和影片演员集合由刮削/NFO/同步维护；LMM 只保留演员显示顺序调整。排序只修改排序字段或关联顺序，不新增演员、不删除演员关联、不修改演员实体。重新刮削后，仍存在的演员保留用户排序，新出现的演员按默认顺序追加。

**已观看边界：** 不新增单独“已观看”开关。播放状态由播放次数、最后播放时间和最近播放表达。
...
| 模块 | 路由 | Bridge 入口 |
|------|------|------------|
| **Tags Category** | `/tags` | 同页横向工具栏：范围、全部/导演/标签/系列/厂商/自定义、排序 |
...
**整理工具：** `/organizer` 是查重与批量整理的统一入口。页面内部左侧固定为“整理工具”，当前包含“重复影片”和“批量整理”两个功能。重复影片阶段复用 `GET /api/duplicates` 结果，展示海报、标题、番号、文件名、文件大小、分辨率、评分、收藏、媒体库、文件路径、真实重复原因和“建议保留”理由；建议只显示，不自动选择。用户必须为选中的每个重复组明确选择一个保留项，其余候选才进入 Safe Delete 预览；执行前重新预览并校验确认令牌，删除仍由 `SafeDeleteWorkflowService` 完成。重复删除前会预览用户个人数据合并：收藏取 OR、自定义标签取并集、播放次数合并、最后播放时间取最新；评分或备注冲突会阻止执行，避免静默覆盖。批量整理阶段复用统一 MovieWall 的 Smart Search、FilterBar、排序、分页、媒体库范围和状态恢复，并在顶部集中显示全选、取消、批量标签、批量收藏、批量评分、批量同步、批量移动、批量重命名和批量删除。批量移动与批量重命名复用 `FileOrganizerService` 的 Dry Run → Preview → Execute → Tasks 链路；普通 MovieWall 选择集不开放任意批量删除，批量删除只通过重复影片 Safe Delete 流程。旧 `/duplicates` 仅保留为兼容重定向，不再作为主导航入口。整理工具不新增数据库、不新增媒体类型、不复制第二套影片查询逻辑；后续批量刮削、同步、标签、收藏和评分都应进入该统一入口。
...
### 24.0 Development Phase Order（固定开发顺序）

所有后续 Sprint 遵循三阶段顺序：

1. **Feature Parity first** — 先完成所有决定保留的旧版用户功能迁移。每完成一个保留功能，必须更新 `docs/migration/FEATURE_PARITY_MATRIX.md` 状态与证据。
2. **Legacy Cleanup second** — Feature Parity 以“保留功能”为准达到 100% 且新实现稳定后，再启动 Legacy Cleanup Sprint，统一删除已经完全替代且无引用的旧代码、旧页面、旧资源和无引用实现；旧数据库迁移、升级兼容、评分/收藏/标签/图片路径等必要兼容逻辑继续保留，除非 Human 明确批准退休。
3. **Product optimization third** — Legacy Cleanup 完成后，项目正式进入产品优化阶段。后续 UI 重构、新功能和体验优化以 Human 的体验需求为最高优先级，不再以旧版一致性为主要目标。

禁止边迁移边大规模重构，禁止边迁移边删除旧实现。旧功能迁移期只做必要修复和最小实现；产品优化期再按 Human 体验需求重新设计。
