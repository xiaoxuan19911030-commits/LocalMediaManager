...
- 详情页显示番号识别信息，并提供重新识别和手动修改番号入口。

- 新增“标准影片库 / 普通媒体库”类型；现有媒体库自动迁移为标准影片库。
- 普通媒体库扫描以文件名作为初始标题，不识别番号，也不创建 MDC-NG、MetaTube 或 JavBus 同步任务。
- 为普通媒体预留基于 MovieId 的封面/截图路径、截图状态、封面来源和 FFmpeg 截图服务接口。
