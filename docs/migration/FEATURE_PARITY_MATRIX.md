...
|---:|---|---|---|---|---|---|---|---|---|---|
| 1 | 新增后自动同步 | P1 | 必须保留 | 已部分迁移 | 扫描/导入；Bridge 同步服务；Sync DTO；Tasks；同步设置 | 数据库写入 | 安装版真实 MetaTube 对照烟测与发布验收 | 0.4.2 | 导入 3 部后产生 3 个同步任务；同步失败不删除影片 | Bridge: 扫描事务入队 + `MetadataSyncExecutor`；Migration: `0004`/`0005`；Automated: 扫描入队与 Provider 测试 11/11；Commit/Smoke/Acceptance 待本批完成 |
| 2 | 同步保存逻辑 | P0 | 必须保留 | 已部分迁移 | 写接口鉴权；Bridge Metadata Service；DTO；事务；来源字段；备份/回滚 | 需要备份与回滚 | 完整回滚 UI、真实 Provider 烟测与发布验收 | 0.4.2 | 同步不覆盖手工标题、标签、评分、收藏 | Bridge: `MetadataWriteService` 非破坏事务；Migration: `0005` 同步快照；Automated: `MetadataWriteFillsEmptyFieldsAndPreservesManualData`；Commit/Smoke/Acceptance 待本批完成 |
...
| 40 | 任务队列 | P0 | 必须保留 | 已部分迁移 | Tasks Schema；Bridge Task Service；Task DTO；Runner；持久日志 | 数据库写入 | 同步/图片/NFO/整理 Runner、跨重启恢复和统一任务控制仍未完成 | 0.5.0 | 完整状态机、重启恢复、失败重试和日志通过 | 部分：GET `/api/tasks`、GET logs、扫描任务 pause/resume/cancel/retry、Tasks UI；Migration `0004`；Automated: 扫描与同步入队测试；Commit: 本次 0.4.2 开发提交待记录；未达到全任务类型完整迁移 |
