...
| 46 | Duplicate Management & Batch Organizer | P1 | 必须保留 | 已部分迁移 | DEC-015；`/organizer`；MovieWall；Duplicate Service；FileOrganizerService；Tasks | 需要备份与回滚 | Safe Delete / organizer execute 第二阶段：重复候选的安全删除、忽略/合并处理、批量移动与批量重命名真实执行 | 0.5.0 | `/organizer` 左侧为整理工具，右侧提供重复影片与批量整理；重复影片 UI 显示海报、文件信息、真实重复原因、展开/收起、全选/取消和非自动选择的保留建议；批量整理复用 MovieWall 并集中显示批量操作栏，移动/重命名/删除为 Disabled 占位 | Bridge: `GET /api/duplicates` 扩展候选展示字段；Batch UI: 复用 `POST /api/videos/batch/favorite|rating|tags|sync`；Migration: N/A；Automated: `DuplicateResultsExposeOrganizerDisplayFieldsAndKeepReasons` + Web build type coverage；Smoke: 本 Sprint 安装版入口待验证；Commit: 本 Sprint `feat(organizer): complete organizer workspace ui` |
...
- UI completion: duplicate mode now exposes poster, title, code, file name, file size, resolution, rating, favorite, media library, file path, true duplicate reason, expand/collapse, select/cancel, and non-binding keep suggestions. Batch mode now exposes one MovieWall-based batch action bar; move, rename, and delete are visible disabled placeholders until Safe Delete / execute migration.
