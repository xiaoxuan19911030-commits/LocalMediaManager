...
| 3 | 删除记忆评分 | P0 | 必须保留 | 尚未迁移 | DeletedRating Migration；Bridge 删除服务；Delete DTO；确认 Dialog；审计 | 删除风险 | 删除命令、记忆表、事务和失败恢复 | 0.4.1 | 删除后只保存文件名+评分；其他用户数据不进入记忆表 | 无完整证据；旧规则 `RememberDeletedRating` 已确认 |
| 4 | 同名恢复评分 | P0 | 必须保留 | 尚未迁移 | #3 Migration；#28 导入流程；Bridge 写入；幂等；回滚 | 数据库写入 | 同名判断、已有评分保护、恢复日志 | 0.4.1 | 同名重导恢复评分；已有新评分不被覆盖 | 无完整证据；旧规则 `RestoreDeletedRating` 已确认 |
| 5 | 收藏兼容 | P0 | 必须保留 | 仅只读 | Bridge UserState Service；Favorite DTO；旧收藏标签映射；事务 | 数据库写入 | 收藏切换与旧特殊标签双向兼容 | 0.4.1 | 旧收藏数量一致；切换后卡片/详情/筛选和重启一致 | 部分：Bridge 只读集合 `/api/collections/favorites`；烟测 479；Commit `83c53b5` |
| 6 | 演员关联 | P0 | 必须保留 | 数据已迁移但操作未迁移 | Bridge Actor Service；Actor DTO；关系事务；去重规则 | 数据库写入 | 编辑、同步写入、去重和刷新一致性 | 0.4.1 | 编辑后关系、演员计数和影片详情即时一致；重启保持 | 部分：GET `/api/entities/actors*`；烟测 1667 演员；Commit `83c53b5` |
| 7 | ActorID=0 修复 | P0 | 必须保留 | 需要重构 | Diagnostics；Repair DTO；Tasks；修复 Migration（如需）；预览/确认/回滚 | 需要备份与回滚 | 修复候选、影响预览、任务日志和撤销 | 0.4.1 | 无 ActorID=0/空关系；不误删有效演员；抽样通过 | 部分：Diagnostics 可识别损坏文本；无修复证据 |
| 8 | 自定义标签 | P0 | 必须保留 | 数据已迁移但操作未迁移 | Bridge Tag Service；Tag DTO；关系事务；系统/用户标签标识 | 数据库写入 | 创建、编辑、删除、单部/批量绑定 | 0.4.1 | 用户标签完整往返；系统标签不可误删；重启保持 | 部分：GET `/api/entities/tags*`；烟测 35 标签；Commit `83c53b5` |
| 9 | 评分 | P0 | 必须保留 | 仅只读 | Bridge UserState Service；Rating DTO；校验；乐观更新回退 | 数据库写入 | 评分写入、清除、错误恢复 | 0.4.1 | 卡片/详情修改一致；重启保持；0 与未评分区分 | 部分：DTO/详情只读显示；Commit `2d52dfc`/`83c53b5` |
| 10 | 收藏 | P0 | 必须保留 | 仅只读 | #5；自动重命名隔离；错误 DTO | 数据库写入 | 收藏写入、取消收藏、重命名失败隔离 | 0.4.1 | 收藏成功不被自动重命名失败阻断 | 部分：只读显示和集合已烟测；无写入测试 |
| 11 | 播放次数 | P0 | 必须保留 | 仅只读 | Bridge Player Service；PlayHistory DTO；成功启动判断；事务 | 数据库写入 | 播放成功后的次数更新 | 0.4.1 | 启动成功 +1；失败不增加；重启保持 | 部分：POST `/api/videos/{id}/play` 可启动；未写次数；Commit `2d52dfc` |
| 12 | 播放历史 | P0 | 必须保留 | 仅只读 | #11；History DTO；位置/时间字段；清理规则 | 数据库写入 | 时间、位置写入与历史清理 | 0.4.1 | 最近播放顺序、次数和位置与实际操作一致 | 部分：GET `/api/collections/history`；烟测 582；Commit `83c53b5` |
| 13 | 上一部/下一部 | P1 | 必须保留 | 尚未迁移 | 查询上下文 DTO；排序/筛选签名；详情缓存/预取 | 无写入 | 结果集上下文、边界和预取 | 0.4.1 | 保持原筛选/排序连续切换；首尾行为明确 | 无完整证据 |
...
- 按新门槛，矩阵中暂时没有功能标记为“已完整迁移”；这是证据标准提升后的正常结果，不代表 0.4.0 页面无效。
- 0.4.1 优先完成 P0 用户状态、标签、评分恢复、演员关系、播放记录与 Tasks 写入基础。
- 0.4.2 集中处理扫描导入、自动同步、MetaTube、NFO、图片和文件整理。
- 0.5.0 完成主要媒体管理功能等价；0.6.0 才开始真实 AI Provider 接入。
