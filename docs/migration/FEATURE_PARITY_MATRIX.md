...
| 31 | Metadata Providers（MetaTube / JavBus） | P1 | 重要 | 已部分迁移 | Plugin/Provider Service；Server Settings；Metadata DTO；Tasks；网络 | 数据库写入 | 敏感 Header/Cookie 安全凭据；少量真实 JavBus 烟测 | 0.5.0 | 单部同步与旧版结果对照；失败保留旧数据 | Bridge: Provider settings/test、POST `/api/videos/{id}/sync`、`IMetadataProvider`/`CompositeMetadataProvider`、`MetaTubeProvider`、`JavBusProvider`；Migration: `0005`；Automated: Provider 优先级/字段映射/设置持久化、JavBus 番号规范化、HTML fixture 解析、缺字段安全失败、404/403/429 分类、Cookie 不进入日志边界；Real smoke: MetaTube 30 部中 22 完成、8 明确无结果、311 图片、22 NFO、用户状态 30/30 与锁定文件 5/5 保持，见 `releases/0.4.3-METATUBE-SMOKE.md`；JavBus 本轮接入现有 Sync/Tasks/Data Center，仍需小样本线上安装版验收；因敏感凭据与真实站点烟测仍缺而保持部分迁移 |
