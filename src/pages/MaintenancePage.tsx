...
export default function MaintenancePage() {
  ...
  return <Box>
    <PageHeader title="Maintenance" description="统一维护中心：健康检查、孤立文件、目录问题、重复影片和缓存维护。"
      action={<Button variant="outlined" startIcon={<RefreshRoundedIcon/>} onClick={() => load(page)}>重新扫描</Button>}/>
    {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}
    {notice && <Alert severity="success" onClose={() => setNotice('')} sx={{ mb: 2 }}>{notice}</Alert>}
    {!data && !error ? <Box sx={{ minHeight: 360, display: 'grid', placeItems: 'center' }}><CircularProgress/></Box> : data && <Stack spacing={2.25}>
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: 1.25 }}>
        ...
        <StatCard label="缺NFO" value={data.stats.missingNfo} icon={<ReportProblemRoundedIcon/>} tone="warning.main"/>
        <StatCard label="缺Metadata" value={data.stats.missingMetadata} icon={<BuildRoundedIcon/>} tone="warning.main"/>
        ...
        {data.issues.length ? <Stack spacing={1} sx={{ maxHeight: 520, overflowY: 'auto' }}>
          {data.issues.map((item, index) => <Card key={`${item.category}-${item.movieId}-${item.path}-${index}`} variant="outlined"><CardContent sx={{ py: 1.25, '&:last-child': { pb: 1.25 } }}>
            <Stack direction="row" spacing={1} useFlexGap sx={{ alignItems: 'center', flexWrap: 'wrap' }}>
              <Chip size="small" color={item.severity === 'error' ? 'error' : item.severity === 'warning' ? 'warning' : 'info'} label={item.category}/>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 2.25 }}>
        <SurfaceSection title="孤立文件" description="图片、NFO、字幕、演员头像和缓存文件中未匹配影片的条目。">
          <Stack spacing={1} sx={{ maxHeight: 360, overflowY: 'auto' }}>{data.orphanFiles.length ? data.orphanFiles.map(item => <Card key={item.path} variant="outlined"><CardContent sx={{ py: 1.25, '&:last-child': { pb: 1.25 } }}><Typography sx={{ fontWeight: 750, overflowWrap: 'anywhere' }}>{item.path}</Typography><Button size="small" onClick={() => openDirectory(item.path)}>定位目录</Button></CardContent></Card>) : <Typography color="text.secondary">Empty</Typography>}</Stack>
        </SurfaceSection>
        <SurfaceSection title="目录问题" description="空目录、只有图片、只有 NFO、只有字幕或没有视频的目录。">
          <Stack spacing={1} sx={{ maxHeight: 360, overflowY: 'auto' }}>{data.directories.length ? data.directories.map(item => <Card key={item.path} variant="outlined"><CardContent sx={{ py: 1.25, '&:last-child': { pb: 1.25 } }}><Typography sx={{ fontWeight: 750, overflowWrap: 'anywhere' }}>{item.path}</Typography><Typography variant="body2" color="text.secondary">{item.reason}</Typography><Button size="small" onClick={() => openDirectory(item.path)}>定位目录</Button></CardContent></Card>) : <Typography color="text.secondary">Empty</Typography>}</Stack>
        </SurfaceSection>
      </Box>
      ...
      <SurfaceSection title="重复影片（二阶段）" description="根据收藏、评分、文件大小、播放次数和最近观看生成保留建议；不执行删除。">
        {data.duplicates.groups.length ? <Stack spacing={1.25}>{data.duplicates.groups.slice(0, 20).map(group => <Card key={`${group.rule}:${group.key}`} variant="outlined"><CardContent>
          <Stack direction="row" spacing={1} sx={{ alignItems: 'center', mb: 1 }}><Chip size="small" label={group.rule}/><Typography sx={{ fontWeight: 850 }}>{group.key}</Typography></Stack>
          {group.items.map(item => <Stack key={item.movieId} direction="row" spacing={1} sx={{ alignItems: 'center', py: .5, borderTop: 1, borderColor: 'divider' }}>
            <Chip size="small" color={item.recommendation === '推荐保留' ? 'success' : 'warning'} label={item.recommendation}/>
