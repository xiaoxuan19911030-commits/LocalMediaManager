...
export default function PluginsPage() {
  const [snapshot, setSnapshot] = useState<SettingsSnapshot>(); const [error, setError] = useState('')
  ...
  return <Box><PageHeader title="插件中心" description="查看旧配置中仍可识别的元数据服务与兼容状态。"/>
    {error && <Alert severity="error">插件配置读取失败：{error}</Alert>}{!snapshot && !error ? <Box sx={{ minHeight: 340, display: 'grid', placeItems: 'center' }}><CircularProgress/></Box> : snapshot && <>
      <Alert severity="info" sx={{ mb: 2 }}>0.4.0 仅建立插件中心基础并读取兼容配置；安装、卸载、启停和在线市场尚未迁移，页面不会假装执行写操作。</Alert>
      {groups.length ? <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 1.5 }}>{groups.map(group => {
        const enabled = group.servers.some(server => server.enabled); const available = group.servers.some(server => server.available > 0)
        return <Card key={group.id}><CardContent><Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, mb: 2 }}><Box sx={{ width: 44, height: 44, display: 'grid', placeItems: 'center', borderRadius: 2, bgcolor: 'action.hover', color: 'primary.main' }}><ExtensionRoundedIcon/></Box><Box sx={{ flex: 1 }}><Typography variant="h6" sx={{ fontWeight: 800 }}>{displayName(group.id)}</Typography><Typography variant="body2" color="text.secondary">旧版兼容 Provider</Typography></Box><Chip size="small" label={enabled ? '已启用' : '未启用'} color={enabled ? 'success' : 'default'}/></Box>
        ...
        </CardContent></Card>})}</Box> : <EmptyState title="没有兼容插件配置" description="当前设置来源中未检测到服务器资源。"/>}
    </>}
  </Box>
}
