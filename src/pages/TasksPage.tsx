...
import { Alert, Box, Button, Card, Checkbox, Chip, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, LinearProgress, MenuItem, Stack, TextField, Tooltip, Typography } from '@mui/material'
...
export default function TasksPage() {
  ...
  const cancelBatch = async () => {
    setBusy(-1); setError('')
    try {
      const result = await bridge.cancelSyncTasks(selectedSyncTasks)
      setNotice(result.message); setSelectedSyncTasks([]); setBatchCancelOpen(false); await load()
    } catch (reason) { setError((reason as Error).message) }
    finally { setBusy(undefined) }
  }
  ...
  return <Box>
    ...
    {!tasks && !error ? <Box sx={{ minHeight: 320, display: 'grid', placeItems: 'center' }}><CircularProgress/></Box> : tasks && <>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 1, alignItems: 'center', flexWrap: 'wrap', mb: 1.5 }}>
        {selectedSyncTasks.length > 0 ? <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}><Typography sx={{ fontWeight: 750 }}>已选择 {selectedSyncTasks.length} 个同步任务</Typography><Button size="small" color="error" variant="outlined" startIcon={<CancelRoundedIcon/>} onClick={() => setBatchCancelOpen(true)}>批量取消</Button><Button size="small" color="inherit" onClick={() => setSelectedSyncTasks([])}>取消选择</Button></Stack> : <Box />}
        <TextField select size="small" label="状态" value={filter} onChange={event => setFilter(event.target.value)} sx={{ width: 150 }}><MenuItem value="all">全部</MenuItem>{Object.entries(statusNames).map(([value, label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}</TextField>
      </Box>
      {visible.length === 0 ? <EmptyState title="暂无任务" description={filter === 'all' ? '扫描、同步等长任务会显示在这里。' : '当前筛选状态下没有任务。'}/> : <Stack spacing={1.25}>{visible.map(task => {
        ...
        return <Card key={task.id} sx={{ p: 2 }}><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '190px 105px minmax(130px,1fr) minmax(170px,1.2fr) 155px auto' }, gap: 2, alignItems: 'center' }}>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>{canBatchCancel && <Checkbox size="small" checked={selectedSyncTasks.includes(task.id)} onChange={(_, checked) => setSelectedSyncTasks(current => checked ? [...current, task.id] : current.filter(id => id !== task.id))} slotProps={{ input: { 'aria-label': `选择同步任务 ${task.name}` } }}/>}<Icon color={complete ? 'success' : failedTask ? 'error' : 'primary'}/><Typography sx={{ fontWeight: 750 }}>{taskNames[task.type] || task.type}</Typography></Box>
