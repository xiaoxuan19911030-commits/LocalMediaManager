...
export default function MetadataPage() {
  const [data, setData] = useState<MetadataOverview>()
  const [error, setError] = useState('')
  const load = useCallback(() => { setError(''); setData(undefined); bridge.metadataOverview().then(setData).catch((reason: Error) => setError(reason.message)) }, [])
  useEffect(load, [load])
  ...
  return <WorkspacePage title="元数据中心" description="只读汇总真实数据库中的刮削、海报、关联和 NFO 状态。" stats={stats} loading={!data && !error} error={error} primaryActions={[refreshAction(load)]}>
    {data && <>
      <SurfaceSection title="完整度" description="数值直接由 Bridge 查询数据库，不在页面中访问 SQLite。"><Box sx={{ display: 'grid', gap: 2 }}>
        ...
        <HealthMeter label="演员关联" value={percent(data.missingActors)} detail={`${data.missingActors} 部缺失`} tone={data.missingActors ? 'warning' : 'success'}/>
        ...
        <HealthMeter label="NFO" value={percent(data.missingNfo)} detail={`${data.missingNfo} 部缺失`} tone={data.missingNfo ? 'warning' : 'success'}/>
      </Box></SurfaceSection>
      <Alert severity="info" icon={<SellRoundedIcon/>} sx={{ mt: 2 }}>本页只做状态聚合；批量刮削、字段覆盖和关系修复继续通过现有任务与维护入口处理。</Alert>
    </>}
  </WorkspacePage>
}
