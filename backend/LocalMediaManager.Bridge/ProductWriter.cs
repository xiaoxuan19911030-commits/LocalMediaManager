...
public sealed record ActorCommand(string Name, string? Alias, int? Gender, string? BirthDate, string? Description,
    int? HeightCm = null, string? Cup = null, string? BirthPlace = null, string? ActivityPeriod = null);
...
public sealed class ProductWriter(string databasePath, RatingHistoryService? ratingHistory = null)
{
    ...
    public async Task<MutationResult> UpdateActorAsync(long actorId, ActorCommand input)
    {
        ...
        await using var transaction = await connection.BeginTransactionAsync();
        string now = Now();
        string sources = ActorProfileService.MarkUserEdited((string?)before.ProfileFieldSourcesJson,
            new[] { "Alias", "BirthDate", "Description", "HeightCm", "Cup", "BirthPlace", "ActivityPeriod" }, now);
        await ExecuteAsync(connection, transaction, "UPDATE Actors SET Name=$name,NormalizedName=$normalized,Alias=$alias,Gender=$gender,BirthDate=$birth,Description=$description,HeightCm=$height,Cup=$cup,BirthPlace=$place,ActivityPeriod=$period,ProfileFieldSourcesJson=$sources,UpdatedAt=$at WHERE Id=$id",
            ("$name", name), ("$normalized", Normalize(name)), ("$alias", input.Alias?.Trim() ?? before.Alias), ("$gender", input.Gender ?? before.Gender),
            ("$birth", input.BirthDate?.Trim() ?? before.BirthDate), ("$description", input.Description?.Trim() ?? before.Description),
            ("$height", input.HeightCm ?? before.HeightCm), ("$cup", input.Cup?.Trim() ?? before.Cup), ("$place", input.BirthPlace?.Trim() ?? before.BirthPlace),
            ("$period", input.ActivityPeriod?.Trim() ?? before.ActivityPeriod), ("$sources", sources), ("$at", now), ("$id", actorId));
    ...
    private static async Task<dynamic?> ReadActorAsync(SqliteConnection c, long id, System.Data.Common.DbTransaction? tx=null) { await using var cmd=c.CreateCommand();cmd.Transaction=tx as SqliteTransaction;cmd.CommandText="SELECT Id,Name,Alias,Gender,BirthDate,Description,HeightCm,Cup,BirthPlace,ActivityPeriod,ProfileFieldSourcesJson FROM Actors WHERE Id=$id";cmd.Parameters.AddWithValue("$id",id);await using var r=await cmd.ExecuteReaderAsync();return await r.ReadAsync()?new {Id=r.GetInt64(0),Name=r.GetString(1),Alias=r.IsDBNull(2)?null:r.GetString(2),Gender=r.IsDBNull(3)?(int?)null:r.GetInt32(3),BirthDate=r.IsDBNull(4)?null:r.GetString(4),Description=r.IsDBNull(5)?null:r.GetString(5),HeightCm=r.IsDBNull(6)?(int?)null:r.GetInt32(6),Cup=r.IsDBNull(7)?null:r.GetString(7),BirthPlace=r.IsDBNull(8)?null:r.GetString(8),ActivityPeriod=r.IsDBNull(9)?null:r.GetString(9),ProfileFieldSourcesJson=r.GetString(10)}:null; }
